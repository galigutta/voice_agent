...
if [ -f "$PIDFILE" ]; then
    ...
    if echo "$WINDOW_CLASS" | grep -qiE '(terminal|konsole|xterm|rxvt|kitty|alacritty|gnome-terminal|terminator|tilix|urxvt|st-256color|wezterm|foot|termius|code|vscode|codium|code-oss)'; then
